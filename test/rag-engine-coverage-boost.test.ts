...
jest.mock('@xenova/transformers', () => ({
  pipeline: jest.fn().mockImplementation(async (type, model, options) => {
...
describe('RAG Engine Coverage Boost - 80% Target', () => {
  ...
  const mockCachePath = '/tmp/test-rag-cache';

  beforeEach(() => {
    jest.clearAllMocks();
    ragEngine = new PersistentRAGEngine(mockCachePath);
  ...
  describe('Domain inference with edge cases', () => {
    it('should infer search domain from query', () => {
      const domains = ragEngine.inferDomains('I need to search for information on the web');
      expect(domains).toContain('search');
    });

    it('should infer communication domain from email/slack keywords', () => {
      const domains = ragEngine.inferDomains('send an email notification to the team slack channel');
      expect(domains).toContain('communication');
    });

    it('should infer cloud infrastructure domain', () => {
      const domains = ragEngine.inferDomains('deploy to aws kubernetes cluster in azure');
      expect(domains).toContain('cloud infrastructure');
    });

    it('should infer command line domain', () => {
      const domains = ragEngine.inferDomains('execute bash script in terminal shell');
      expect(domains).toContain('command line');
    });

    it('should handle multiple domain matches', () => {
      const domains = ragEngine.inferDomains('create a react component to query database and deploy to aws');
