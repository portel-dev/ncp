 ...
 * Uses typescript-json-schema or ts-morph for type reflection
 */
...
/**
 * Extract schemas from a SimpleMCP class file
 */
export class SchemaExtractor {
  ...
  async extractFromFile(filePath: string): Promise<ExtractedSchema[]> {
    try {
      ...
      logger.error(`Failed to extract schemas from ${filePath}: ${error.message}`);
      return [];
    }
  }
  ...
  extractFromSource(source: string): ExtractedSchema[] {
    ...
    // We need to handle nested braces in the params type
    // The (?:^|\n) ensures we only match JSDoc that starts at beginning of line
    const methodRegex = /\/\*\*\s*\n([\s\S]*?)\*\/\s+async\s+(\w+)\s*\(/g;
  ...
  private parseParamTypes(
    ...
    // Split by commas (but not inside nested objects/arrays)
