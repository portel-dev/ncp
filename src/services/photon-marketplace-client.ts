/**
 * Photon Marketplace Client - Manage multiple Photon marketplaces
 *
 * Integrates with Photon's marketplace system to query and install Photons
 * from the official portel-dev/photons repository and user-defined marketplaces
 */
...
- Local path: ./path/to/marketplace or /absolute/path`
  ...
  async fetchManifest(marketplace: Marketplace): Promise<MarketplaceManifest | null> {
    try {
      ...
      logger.debug(`Failed to fetch manifest from ${marketplace.name}: ${error.message}`);
    }
